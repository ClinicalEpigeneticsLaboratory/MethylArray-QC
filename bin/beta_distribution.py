...
def main():
    if len(sys.argv) != 3:
        print("Usage: python beta_distribution.py <path_to_imputed_mynorm> <n_cpgs>")
        sys.exit(1)

    path_to_imputed_mynorm = Path(sys.argv[1])
    n_cpgs = int(sys.argv[2])
    ...
    cpgs_to_plot = random.sample(imputed_mynorm.index.to_list(), n_cpgs)
    plot_data = imputed_mynorm.filter(items = cpgs_to_plot, axis = 0)
    plot_data = plot_data.T

    # Prepare figure
    plot_title = f'Density plot of imputed data (randomly selected {n_cpgs} CpGs)'
