...
def validateParamsCpus(cpus: int) -> None:
    ...
    assert cpus <= max_available_cpus, "params.cpus cannot be larger than $max_available_cpus!"

def validateParamsPCA(sample_sheet_path: Path, pca_number_of_components: int, pca_matrix_PC_count: int) -> None:
    sample_sheet = pd.read_csv(sample_sheet_path)
    sample_count = sample_sheet.index.size
    
    assert pca_number_of_components <= sample_count, f"params.pca_number_of_components must be below {sample_count} (sample count)!"
    assert pca_matrix_PC_count <= sample_count, f"params.pca_matrix_PC_count must be below {sample_count} (sample count)!"
    assert pca_matrix_PC_count <= pca_number_of_components, f"params.pca_matrix_PC_count ({pca_matrix_PC_count}) must be <= params.pca_number_of_components ({pca_number_of_components})!"

def main():
    if len(sys.argv) != 6:
        print("Usage: python additional_validators.py <params_input> <params_sample_sheet> <params_cpus> <params_pca_number_of_components> <params_pca_matrix_PC_count>")
        sys.exit(1)

    params_input = Path(f"{sys.argv[1]}")
    params_sample_sheet = Path(f"{sys.argv[2]}")
